...
function getTaskCategory(task) {
  return timeStore.categories.find(c => c.id === task.category) || {
    color: '#6b7280',
    icon: '📌'
  }
}
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 pt-16">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
        <div class="animate-fade-in">
          ...
          <p class="text-gray-400 mt-1">
            <span class="text-indigo-400 font-semibold">{{ totalWeekTasks }}</span> tarefas planejadas •
            <span class="text-emerald-400 font-semibold">{{ completedWeekTasks }}</span> concluídas
          </p>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showTaskForm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" @click="showTaskForm = false" />
